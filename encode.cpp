...
{   
    ...
    std::unique_ptr<QUANG::octree> root = std::make_unique<QUANG::octree>(min_bound, max_bound);// Tạo cây octree với std::unique_ptr để tự động quản lý bộ nhớ

    QUANG::from_pc_to_octree(&test, root.get(), error);// Chuyển đổi đám mây điểm thành cây octree với độ sai số error
  
    QUANG::write_octree_to_file(root.get(), argv[3]);// Lưu cây octree vào file đầu ra (argv[3])
